...
@section('content')
    <h4 class="title_page">Danh sách chi tiết công việc "{{$task->name}}"</h4>
    <a href="task/list" class="btn btn-primary active" role="button" aria-pressed="true" style="margin-bottom: 10px">Quay lại</a>
    <a href="taskchild/add/{{$task->id}}" class="btn btn-primary active" role="button" aria-pressed="true" style="margin-bottom: 10px">Thêm công việc con</a>
    <a href="taskchild/list" class="btn btn-primary active" role="button" aria-pressed="true" style="margin-bottom: 10px">DS công việc con</a>
    @if(session('thongbao'))
    ...
    <table class="table table-bordered td_center">
        ...
        @foreach($task->task_childs as $task_child)
            <tr>
                <th scope="row">{{$task_child->id}}</th>
                <td>{{$task_child->name}}</td>
                <td>{{$task_child->content}}</td>
